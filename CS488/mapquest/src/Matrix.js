...
export class Matrix3 {
    ...
    multiplyVector(vec) {
        var ret = new Vector3();
        ret.x = (this.v00 * vec.x) + (this.v01 * vec.y) + (this.v02 * vec.z);
        ret.y = (this.v10 * vec.x) + (this.v11 * vec.y) + (this.v12 * vec.z);
        ret.z = (this.v20 * vec.x) + (this.v21 * vec.y) + (this.v22 * vec.z);
        return ret;
    }
...
export class Matrix4 {
    ...
    createPerspectiveProjectionMatrix4(aspectRatio, FOV, near, far)
    {
        var ret = new Matrix4();
        var yScale = 1.0 / Math.tan((FOV / 2.0) * (Math.PI / 180.0)) * aspectRatio;
        var xScale = yScale / aspectRatio;
        var frustumLength = far - near;
        ret.v00 = xScale;
        ret.v11 = yScale;
        ret.v22 = -((near + far) / frustumLength);
        ret.v32 = -1;
        ret.v23 = -((2 * near * far) / frustumLength);
        ret.v33 = 0;
        return ret;
    }
